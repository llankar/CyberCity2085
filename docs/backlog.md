...
- [x] BATTLE-UI-GODOT Combat mission UI handoff: mission briefing now exports selected mission/squad/map state to `runtime/godot_combat/mission_handoff.json`, resolves the project and handoff paths before launch so Godot reads the written payload, uses the in-app Settings screen file browser path before environment variables, persists that executable path immediately after selection, and the dedicated `godot/combat_missions_ui` project now renders a live tactical HUD with mission header, grid, initiative, action deck, target lock, and combat log when Godot is available, while still falling back to the Arcade battle view when no Godot executable is configured. Movement stays free-form in this slice; no blocking layer is included. (completed June 7, 2026)
